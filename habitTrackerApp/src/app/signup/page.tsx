...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (!signUp) {
      setMessage('Signup service not available')
      setLoading(false)
      return
    }
    ...
    try {
      ...
      if (result.status === 'complete') {
        ...
        console.log('Signup needs verification, status:', result.status)
        ...
        try {
          ...
          setMessage('Account created but email verification failed. Please try logging in.')
        }
      }
    } catch (err: any) {
  ...
  const handleVerification = async (e: React.FormEvent) => {
    ...
    if (!signUp) {
      setMessage('Verification service not available')
      setLoading(false)
      return
    }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center text-black">
      <div className="max-w-md w-full space-y-8 p-8">
        ...
        {message && (
          <p className={`text-center ${message.includes('failed') || message.includes('error') ? 'text-red-600' : 'text-green-600'}`}>
            {message}
          </p>
        )}
      </div>
